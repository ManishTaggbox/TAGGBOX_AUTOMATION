...
class SocialCard {
    constructor(page) {
        this.page = page;
        this.socialCardBtn = page.locator("//p[normalize-space()='Social Card']");
    ...
    async socialCard() {
        ...
        const dynamicName = 'SocialCard';

        await test.step("Create a website with name 'SocialCard'", async () => {
            await createWebsite.createWebsite(dynamicName);
        });

        await test.step("Click on 'Social Card' button", async () => {
            await this.waitForElementAndClick(this.socialCardBtn);
        });
