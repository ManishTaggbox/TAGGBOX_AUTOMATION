...
class Collage {
    ...
    get collageBtn() { return this.page.locator("//p[@class='mb-0 text-center fs-10'][normalize-space()='Collage']"); }
    ...
    async collage() {
        ...
        const dynamicName = 'Collage';

        await test.step("Create a website with name 'Collage'", async () => {
            await createWebsite.createWebsite(dynamicName);
        });

        await test.step("Click on 'Collage' button", async () => {
            await this.waitForElementAndClick(this.collageBtn);
        });
