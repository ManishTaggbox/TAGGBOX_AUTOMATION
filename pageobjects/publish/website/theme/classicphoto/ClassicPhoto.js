...
class ClassicPhoto {
    constructor(page) {
        this.page = page;
        this.classicPhotoBtn = page.locator("//p[normalize-space()='Classic Photo']");
        this.customization = page.locator("//a[@data-rr-ui-event-key='customization']");
        this.cardToggle = page.locator("(//input[@id='inherit_'])[1]");
        this.fontFamily = page.locator("(//div[contains(@class,'selectpicker__control css-13cymwt-control')])[1]");
    ...
    async classicPhoto() {
        ...
        const dynamicName = 'ClassicPhoto';

        await test.step("Create a website with name 'ClassicPhoto'", async () => {
            await createWebsite.createWebsite(dynamicName);
        });

        await test.step("Click on 'Classic Photo' button", async () => {
            await this.waitForElementAndClick(this.classicPhotoBtn);
        });
