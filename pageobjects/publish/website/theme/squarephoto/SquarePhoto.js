...
class SquarePhoto {
    ...
    get squarePhotoBtn() { return this.page.locator("//p[normalize-space()='Square Photo']"); }
    ...
    async squarePhoto() {
        ...
        const dynamicName = 'SquarePhoto';

        await test.step("Create a website with name 'SquarePhoto'", async () => {
            await createWebsite.createWebsite(dynamicName);
        });

        await test.step("Click on 'Square Photo' button", async () => {
            await this.waitForElementAndClick(this.squarePhotoBtn);
        });
