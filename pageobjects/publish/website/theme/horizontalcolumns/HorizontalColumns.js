...
class HorizontalColumns {
    constructor(page) {
        this.page = page;
        this.horizontalColumnsBtn = page.locator("//p[normalize-space()='Horizontal Columns']");
        ...
        this.socialActions = page.locator('#social_actions');
        this.moreActions = page.locator("//button[normalize-space()='More actions']");
        this.showMore = page.locator("input[id='Showmore button_']");
    ...
    async horizontalColumns() {
        const createWebsite = new CreateWebsite(this.page);
        const dynamicName = 'HorizontalColumns';

        await test.step("Create a website with name 'HorizontalColumns'", async () => {
            await createWebsite.createWebsite(dynamicName);
        });

        await test.step("Click on 'Horizontal Columns' button", async () => {
            await this.waitForElementAndClick(this.horizontalColumnsBtn);
        });
        ...
        await test.step("Set Author color to #704c60", async () => {
         ...
         await test.step("Enable showMore feature", async () => {
            ...
            await this.waitForElementAndClick(this.showMore);
        });
