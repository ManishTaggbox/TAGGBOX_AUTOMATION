...
class HorizontalSlider {
    ...
    get horizontalSliderBtn() { return this.page.locator("//p[normalize-space()='Horizontal Slider']"); }
    ...
    async horizontalSlider() {
        const createWebsite = new CreateWebsite(this.page);
        const dynamicName = 'HorizontalSlider';

        await test.step("Create a website with name 'HorizontalSlider'", async () => {
            await createWebsite.createWebsite(dynamicName);
        });

        await test.step("Click on 'Horizontal Slider ' button", async () => {
            await this.waitForElementAndClick(this.horizontalSliderBtn);
        });
