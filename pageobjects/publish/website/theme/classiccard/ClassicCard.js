...
class ClassicCard {
    ...
    get classicCardBtn() { return this.page.locator("//p[@class='mb-0 text-center fs-10'][normalize-space()='Classic Card']"); }
    ...
    get iconColor() { return this.page.locator('#icon_color'); }
    get iconColorClose() { return this.page.locator("label[for='Defaultsocial icon color_'] span[class='me-2']"); }
    ...
    async classicCard() {
        ...
        const dynamicName = 'Classic Card';

        await test.step("Create a website with name 'Classic Card'", async () => {
            await createWebsite.createWebsite(dynamicName);
        });

        await test.step("Click on Classic Card' button", async () => {
            await this.waitForElementAndClick(this.classicCardBtn);
        });
        ...
        await test.step("Set card color to #704c60", async () => {
            await this.cardColor.scrollIntoViewIfNeeded();
