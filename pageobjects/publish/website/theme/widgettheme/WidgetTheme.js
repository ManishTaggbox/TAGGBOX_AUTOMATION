...
class WidgetTheme {
    ...
    get widgetThemeBtn() { return this.page.locator("//p[normalize-space()='Widget Theme']"); }
    ...
    async widgetTheme() {
        ...
        const dynamicName = 'WidgetTheme';

        await test.step("Create a website with name 'WidgetTheme'", async () => {
            await createWebsite.createWebsite(dynamicName);
        });

        await test.step("Click on 'Widget Theme' button", async () => {
            await this.waitForElementAndClick(this.widgetThemeBtn);
        });
