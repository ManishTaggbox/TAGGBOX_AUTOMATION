...
class YoutubeShorts {
    constructor(page) {
        ...
        this.shortsTab = page.locator('#YoutubeFeedTab-tab-Shorts');
        this.urlField = page.locator('#feedsfield-Shorts');
    ...
    async youtubeShorts() {
        ...
        await test.step('Step 3: Click to "Shorts" tab', async () => {
            await this.shortsTab.waitFor({ state: 'visible', timeout: 10000 });
            await this.shortsTab.click();

            console.log('✅ Clicked on "Shorts" tab');
        });

        await test.step('Step 4: Wait for shorts url input, then fill', async () => {
            await this.urlField.waitFor({ state: 'visible', timeout: 10000 });
            await this.urlField.fill(YOUTUBE.YOUTUBESHORTSURL);

            console.log('✅ Filled YouTube shorts URL');
        });
