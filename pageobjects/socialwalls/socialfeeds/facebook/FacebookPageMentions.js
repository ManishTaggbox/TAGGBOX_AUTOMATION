...
class FacebookPageMentions {
    constructor(page) {
        this.page = page;
        this.facebook = page.locator("//li[contains(@class,'facebook')]//button[contains(@type,'button')]");
        this.mentionsTab = page.locator("//span[text()='Page Mentions']").last();
        this.dropdownIcon = page.locator(".selectpicker__indicators").last();
        this.dropdownOption = page.locator(".selectpicker__option");
    ...
    async facebookPageMentions() {
        ...
        await test.step('Step 4: Click to "Page Mentions" tab', async () => {
            await this.mentionsTab.waitFor({ state: 'visible', timeout: 10000 });
            await this.mentionsTab.click();

            console.log('✅ Clicked on "Page Mentions" tab');
        });

        await test.step('Step 5: Click to select dropdown option', async () => {
            await this.dropdownIcon.waitFor({ state: 'visible', timeout: 10000 });
            await this.dropdownIcon.click();

            await this.dropdownOption.nth(1).waitFor({ state: 'visible', timeout: 10000 });
            await this.dropdownOption.nth(1).click();

            console.log('✅ Clicked to select dropdown option');
        });
...
export default FacebookPageMentions;