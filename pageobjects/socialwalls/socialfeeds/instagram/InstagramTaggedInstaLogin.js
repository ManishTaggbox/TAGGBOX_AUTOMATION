...
class InstagramTaggedInstaLogin
{
    constructor(page) 
    {
        ...
        this.taggedTab = page.locator('#CreateFeedTab-tab-tagged');
    ...
    async instagramTaggedInstaLogin() 
    {
        ...
        await test.step('Step 4: Click on "Tagged(Insta Login)" tab', async () => 
        {
            await this.taggedTab.waitFor({ state: 'visible', timeout: 10000 });
            await this.taggedTab.click();

            console.log('✅ Clicked on "Tagged(Insta Login)" tab');
        });
...
export default InstagramTaggedInstaLogin;
