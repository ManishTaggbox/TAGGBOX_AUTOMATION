...
class NullField
{
    ...
    async nullField() 
    {
        ...
        await test.step("Step 2: Click on Login btn with null fields", async () => 
        {
        ...
        await test.step("Step 3: Assert the validation msg for Email field", async () => 
        {
        ...
        await test.step("Step 4: Assert the validation msg for Password field", async () => 
        {
        ...
        // Enter Email only 
        await test.step("Step 5: Enter Email only and click Login btn", async () => 
        {
            await this.emailField.waitFor({state: 'visible', timeout: 5000});
        ...
        await test.step("Step 6: Assert the validation msg on entering email only", async () => 
        {
        ...
        // Enter Password only
        await test.step("Step 7: Enter password only and click Login btn", async () => 
        {
            await this.emailField.waitFor({state: 'visible', timeout: 5000});
            await this.emailField.fill('');
        ...
        await test.step("Step 8: Assert the validation msg on entering password only", async () => 
        {
            await this.validationMsg.waitFor({state: 'visible', timeout: 15000});
