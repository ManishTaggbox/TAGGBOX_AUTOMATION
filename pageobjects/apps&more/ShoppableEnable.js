...
class ShoppableEnable
{
    constructor(page) 
    {
        this.page = page;
        this.appsMoreMenu = page.locator('#apps_more');
        this.enableBtn = page.locator('//button[text()="Enable"]');

        

        this.toastMsg  = page.locator('');
    ...
    async shoppableEnableConditions() 
    {
        ...
        await test.step("Step 2: Assert 'Enable' button is displayed and clicked", async () => 
        {
            await this.enableBtn.waitFor({ state: 'visible', timeout: 5000 });
            await this.enableBtn.click();
            
            console.log("✅ 'Enable' button is displayed and clicked");
        });
...
export default ShoppableEnable;
